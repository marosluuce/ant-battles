...
render (width, height) {ants, food, nests} =
    let renderedBg = background 51 51 scale
        renderedAnts = List.map (renderAnt scale) ants
        renderedFood = List.map (renderFood scale) food
        renderedHive = box scale (0, 0) Color.black
        elements = [renderedBg] ++ renderedAnts ++ renderedFood ++ [renderedHive]
    in
        toHtml <| collage width height elements

renderColorSquare : Nest -> Html a
renderColorSquare {id} =
  let nestColor = randomColor id
      colorSquare = square scale |> filled nestColor
  in toHtml <| collage 10 10 [colorSquare]

background : Float -> Float -> Float -> Form
background blocksWide blocksHigh scale =
  let width = blocksWide * scale
...
renderAnt scale ant =
  let antColor = randomColor ant.nestId
      ...
      renderedName = scaledText "F" scale ant.position
  in
      if ant.hasFood then
...
scaledText string scale (x, y) =
  let position = (x * scale, (y * scale) + (scale / 8))
      displayed = Text.fromString string
  in Text.fromString string
